...
function createDictionary(data, otherData){
    ...
    for (let element of myData){
        ...
        new_data.push({"district": replaceChars(element.values[0].district),
            "polygon": parsePolygon(element.values[0].polygon), "neighbourhoods": neighbourhoods, "housing_2018": element.values[0].housing_2018})
    }
    console.log(new_data);
    return new_data;
}

function createCrimeDictionary(data){
    let myData = get_all_by_district(data);
    let new_data = [];
    for (let element of myData){
        let displayStat = [];
        for (let thing of displayStat){
            if (element.values[0].district === thing.values[0].district){
                displaystat.push({
                    "crime_index_2017": replaceChars(thing.values[0].crime_index_2017)
                    , "polygon": parsePolygon(thing.values[0].polygon)});
            }
        }
        new_data.push({"district": replaceChars(element.values[0].district),
            "polygon": parsePolygon(element.values[0].polygon), "housing_2018": element.values[0].housing_2018})
    }
    console.log(new_data);
    return new_data;
}
